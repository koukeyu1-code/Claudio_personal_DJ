...
function dynamicContext({ recentPlays = [], weeklyTop = [], unplayable = [], candidates = [], weather = null, routines = null, schedule = null, tasks = [], dayRest = null } = {}) {
  ...
  if (typeof dayRest === 'boolean') {
    ...
    ctx += `\n\n【重要·时段必须以当前时间为准】现在是「${slot}」。对话记录里你之前的串场和剧集名（比如带"深夜""午间""午后"等字样的）可能来自**别的时间段**，**一律以这里的「${slot}」为准**——不要照抄历史里的时段措辞、语气或旧剧集名。如果当前「${slot}」和你上一条的剧集名/时段语气对不上，就**重新起一个贴合「${slot}」的剧集名和口吻**，绝不沿用与此刻时段不符的旧名（例如午后就别再叫"深夜…"）。`;
  }
  if (weather) {
